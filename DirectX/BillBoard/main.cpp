...
LPDIRECT3DTEXTURE9			g_pTexDiffuse1 = NULL;
...
HRESULT InitTexture()
{
	...
	if (FAILED(D3DXCreateTextureFromFile(g_pd3dDevice, "tile2.tga", &g_pTexDiffuse)))
		return E_FAIL;	
	if (FAILED(D3DXCreateTextureFromFile(g_pd3dDevice, "banana.bmp", &g_pTexDiffuse1)))
		return E_FAIL;
...
HRESULT InitVB()
{
	...
	if (FAILED(g_pd3dDevice->CreateVertexBuffer(6 * sizeof(CUSTOMVERTEX), 0, D3DFVF_CUSTOMVVERTEX,
		D3DPOOL_DEFAULT, &g_pVB1, NULL)))
		return E_FAIL;

	g_pTexHeight->LockRect(0, &d3drc, NULL, D3DLOCK_READONLY);
	VOID* pVertices1;
	if (FAILED(g_pVB1->Lock(0, 3 * sizeof(CUSTOMVERTEX), (void**)&pVertices1, 0)))
		return E_FAIL;
	...
	memcpy(pVertices1, &v1, sizeof(CUSTOMVERTEX) * 3);
	g_pVB1->Unlock();
...
VOID Animate()
{
	...
	D3DXMatrixIdentity(&matBillboardMatrix);

	D3DXMATRIXA16 * matView;
	matView = ZCamera::GetInstance()->GetViewMatrix();
	matBillboardMatrix._11 = matView->_11;
	matBillboardMatrix._13 = matView->_13;
...
void DrawTRIANGLEMesh(D3DXMATRIXA16* pMat)
{
	g_pd3dDevice->SetTransform(D3DTS_WORLD, pMat);
...
void Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		g_pd3dDevice->SetTexture(0, g_pTexDiffuse1);
		g_pd3dDevice->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
		...
		DrawTRIANGLEMesh(&matBillboardMatrix);
