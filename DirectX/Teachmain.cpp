...
void SetupPlanet()
{
	Sun = new Planet(nullptr, 0.0f,3.f);
	Earth = new Planet(Sun, 1.0f, 3.f);
	Moon = new Planet(Earth, 1.0f, 3.f);
	Mars = new Planet(Sun, 1.0f, 6.f);
}
...
VOID Animate()
{
	...
	//Sun->Animate(t);
	//Earth->Animate(GetTickCount() / 500.0f);
	//Moon->Animate(GetTickCount() / 500.0f);
	//Mars->Animate(GetTickCount() / 500.0f);
...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		matWorld =  g_matTMChild * g_matRChild * matWorld;
		DrawMesh(&matWorld);

		matWorld =  g_matTMChild2 * g_matRChild2 * matWorld;
		DrawMesh(&matWorld);		
